...
/// The output sample rate an AAC-LC `sampling_frequency_index` decodes to.
///
/// `None` for the three MPEG-4 reserved index values (13-15) and for any
/// value outside the table entirely; `validate()` reports the latter as a
/// separate `Finding` rather than this lookup silently substituting a rate.
#[must_use]
pub(crate) fn aac_lc_output_sample_rate(sampling_frequency_index: u8) -> Option<u32> {
    AAC_LC_SAMPLE_RATES
