...
- Residuals: (1) The flat model stores one `SequenceObu` (192 B on the 64-bit targets probed) per OBU. This generic per-OBU class is the dominant amplification: 2-byte OBUs (`20 00`, `30 00`, `c0 00`) give 96× live and 3.27-3.66 GB max RSS at 32 MiB. Nothing but input length bounds it, because the 2 MiB cap is per OBU; it was not in earlier residual lists. Resolved by quick 260914-hoa as streaming, not as a budget: `SequenceReader` yields one OBU at a time, and the same 32 MiB measured 34.3 MB max RSS with items dropped (`parse_sequence` 3.25 GB on the same run, macOS x86_64). `parse_sequence` keeps the whole-file factor and documents it. The in-payload classes are linear and capped per OBU. Measured live factors: `read_strings` about 42× (about 50× with malloc rounding), AE extension params about 34×, Mix Gain Step subblocks and layouts about 17×, substream ids about 4×; single 2 MiB OBU peaks reach about 55× (`read_strings`). Since quick 260914-5c5, an extension definition (`param_definition_type` > 2) never governs a block, so `parse_sequence` keeps such blocks as one raw `Vec<u8>` (`UngovernedParameterBlock`). `ParameterData::Raw` subblocks (about 40× exact / 64× with `Vec` growth slack) are reachable only when a caller registers a `ParameterDataContext::Reserved` context and calls `read_parameter_block` directly. A whole-file `ParseLimits` budget is deferred (260914-hoa deferred item 1), and the registry still grows with redundant descriptor copies (260914-hoa deferred item 3). This is accepted; it is not count-driven. (2) No allocator-level measurement exists, because `unsafe_code` is `forbid` and a counting global allocator needs `unsafe`; an RSS-limited fuzz run (`-rss_limit_mb`) remains the only peak-memory check.
