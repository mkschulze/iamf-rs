...
- Residuals: (1) The model still stores one element per decoded item, so memory stays proportional to consumed input bytes within the 2 MiB OBU cap (for example about 24 bytes per empty annotation string). This is accepted; it is not count-driven. (2) No allocator-level measurement exists, because `unsafe_code` is `forbid` and a counting global allocator needs `unsafe`; an RSS-limited fuzz run (`-rss_limit_mb`) remains the only peak-memory check.
