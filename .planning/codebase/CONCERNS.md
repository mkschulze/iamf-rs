...
- Residuals: (1) The model still stores one element per decoded item, so memory stays proportional to consumed input bytes within the 2 MiB OBU cap. The remaining linear classes, all at least one wire byte per element and bounded by that cap, are: extension (Raw, `param_definition_type` >= 3) Parameter Block subblocks at about 40× exact / 64× with `Vec` growth slack; Mix Gain Step subblocks at about 13×/16×; and `read_strings` NUL strings at about 24-32×. The review's 38 MB -> 1.94 GB RSS scenario is therefore still reachable through extension-type blocks. A per-parse memory budget is the only hard bound and is an open user decision (260914-1mq deferred item 1). This is accepted; it is not count-driven. (2) No allocator-level measurement exists, because `unsafe_code` is `forbid` and a counting global allocator needs `unsafe`; an RSS-limited fuzz run (`-rss_limit_mb`) remains the only peak-memory check.
