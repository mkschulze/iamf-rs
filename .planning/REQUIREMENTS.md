...
- [ ] **API-03**: Minimum-profile selection evaluates every Mix Presentation independently and writes the highest profile required by any one Presentation into the sequence header. It never selects from the union of elements across unrelated Presentations, and it supports the known expanded loudspeaker layouts needed for profile counting while rejecting reserved layouts
  - *Amendment (quick 260913-qk3, user directive 2026-09-13 "follow the spec"):* the per-presentation reading is replaced by the IAMF v1.1.0 scopes (v1.0.0-errata Simple/Base sections as adopted). The Simple/Base channel limits (16/18) and the Base-Enhanced 28-element limit are per Mix Presentation; the Simple/Base unique-element limits (1/2), Base's at-most-one scene-based and at-most-one multi-layer channel-based element, and the Base-Enhanced 28-channel total span the whole IA Sequence, unreferenced elements included. The header carries the highest requirement. Checkbox status unchanged.
- [ ] **API-04**: The high-level configuration supports ordered multiple Mix Presentations, multiple valid sub-mixes at the generic IAMF layer, shared channel-based or scene-based Audio Elements, per-element rendering and mix gain, per-sub-mix output gain, layouts, labels and caller-supplied loudness. Shared Audio Elements and substreams are described and emitted once
  - *Amendment (quick 260913-qk3):* under IAMF v1.1.0 `num_sub_mixes` SHOULD be 1 (index.bs:1920) and both pinned references reject any other count (iamf-tools `profile_filter.cc:220-238`, libiamf `IAMF_OBU.c:760-782`), so `build()` rejects a Mix Presentation with 0 or more than 1 sub-mixes with `SubMixCountNotOne`. The low-level model still parses and round-trips multi-sub-mix foreign files. Checkbox status unchanged.
...
- [ ] **API-06**: The frame API accepts LPCM frame payloads or already encoded FLAC/Opus/AAC-LC access units together with their typed codec/frame plan. It validates codec identity, sample rate, samples per frame, substream coverage, trim agreement and temporal-unit alignment; it never performs codec encoding, resampling, panning, rendering or loudness measurement
  - *Amendment (quick 260914-pyd, merged from `feature/aac-lc-framing`, 2026-09-14):* AAC-LC joined FLAC/Opus as a fourth framing-only codec — this crate accepts caller-supplied pre-encoded `raw_data_block()` access units and never encodes or decodes AAC-LC. Checkbox status unchanged.
...
- [ ] **API-09**: Public-API-only contract fixtures cover ordinary Stereo, mixed channel-based plus HOA scene elements, multiple ordered Presentations sharing an Audio Element, explicit element/output gains and loudness, pre-decimated parameter blocks, and LPCM/FLAC/Opus/AAC-LC framing. Each emitted deliverable passes the existing structural, deterministic and external-conformance gates applicable to its codec
  - *Amendment (quick 260914-pyd, 2026-09-14):* AAC-LC framing is now covered (`tests/fixtures/reference/test_000076_aac_lc.iamf`), gated the same as FLAC/Opus. Checkbox status unchanged.
...
- **DECO-03**: The AAC-LC decoder-config wire syntax and Audio Frame framing now live here (quick
  260914-pyd, merged from `feature/aac-lc-framing`, 2026-09-14) — this crate frames caller-supplied
  pre-encoded `raw_data_block()` access units, matching the boundary already used for FLAC/Opus. The
  AAC codec implementation (encode/decode) remains outside this crate, still `iamf-decode-rs`'s job
